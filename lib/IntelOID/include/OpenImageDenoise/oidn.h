...
OIDN_API int oidnGetNumPhysicalDevices(void);
...
OIDN_API bool oidnIsCPUDeviceSupported(void);
...
OIDN_API OIDNBuffer oidnNewSharedBufferFromWin32Handle(
	OIDNDevice device, OIDNExternalMemoryTypeFlag handleType, void* handle, const void* name, size_t byteSize);
...
OIDN_API void oidnSetFilterImage(OIDNFilter filter, const char* name, OIDNBuffer buffer, OIDNFormat format, size_t width, size_t height,
...
// If pixelByteStride and/or rowByteStride are zero, these will be computed automatically.
OIDN_API void oidnSetSharedFilterImage(OIDNFilter filter, const char* name, void* devPtr, OIDNFormat format, size_t width, size_t height,
	size_t byteOffset, size_t pixelByteStride, size_t rowByteStride);
...
OIDN_API void oidnExecuteSYCLFilterAsync(OIDNFilter filter, const sycl::event* depEvents, int numDepEvents, sycl::event* doneEvent);
#endif
