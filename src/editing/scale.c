...
float	eval_scale_magnitude(t_context *ctx, t_vec2i delta, float speed)
{
	...
	obj_screen_pos = world_to_screen(ctx, &ctx->scene.cam, &ctx->scene.cam.viewport, ctx->editor.selected_obj->transform.pos);
	dist_prev = sqrtf((v_mouse.x - obj_screen_pos.x) * (v_mouse.x - obj_screen_pos.x) +
						(v_mouse.y - obj_screen_pos.y) * (v_mouse.y - obj_screen_pos.y));
	v_mouse.x += (float)delta.x;
	v_mouse.y += (float)delta.y;
	dist_curr = sqrtf((v_mouse.x - obj_screen_pos.x) * (v_mouse.x - obj_screen_pos.x) +
						(v_mouse.y - obj_screen_pos.y) * (v_mouse.y - obj_screen_pos.y));
