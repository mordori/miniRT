...
vec3_add(vec3_scale(r->cam.viewport.d_u, (float)mouse.x + 0.5f), vec3_scale(r->cam.viewport.d_v, (float)mouse.y + 0.5f)));
	ray = new_ray(r->cam.transform.pos, vec3_normalize(vec3_sub(pixel_loc, r->cam.transform.pos)));
	hit = new_hit(0);
	if ((int)hit_bvh_editing(ctx->scene.geo.bvh_root_idx, &ray, &hit, ctx->scene.geo.bvh_nodes) |
		(int)hit_planes(ctx, &ray, &hit))
		select_obj(ctx, r, &ray, &hit);
	else if (ctx->editor.selected_obj && !(int)hit_object(ctx->editor.selected_obj, &ray, &hit))
