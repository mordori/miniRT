...
void	random_uv(const t_context *ctx, t_path *path, t_pixel *pixel, t_bn_channel c)
{
	...
	if (path->bounce == 0)
	{
		path->uv = vec2(blue_noise(&ctx->tex_bn, pixel, dim_u), blue_noise(&ctx->tex_bn, pixel, dim_v));
	}
	else
	{
		offset = vec2(static_blue_noise(&ctx->tex_bn, pixel, dim_u), static_blue_noise(&ctx->tex_bn, pixel, dim_v));
