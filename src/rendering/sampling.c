...
t_vec3 sample_cos_hemisphere(t_vec3 n, t_vec2 uv) {
	float cos_theta = sqrtf(uv.v);
	float sin_theta = sqrtf(fmaxf(0.0f, 1.0f - uv.v));
...
t_vec3 sample_cone(t_vec3 vec, float cos_theta_max, t_vec2 uv) {
	float cos_theta = 1.0f + uv.v * (cos_theta_max - 1.0f);
	float sin_theta = sqrtf(fmaxf(0.0f, 1.0f - cos_theta * cos_theta));
	return spherical_to_world(vec, uv.u, sin_theta, cos_theta);
}
...
void sample_ggx_vndf(t_path* path, float alpha, t_vec2 uv) {
	...
	t_vec3 wi_std = vec3(vec3_dot(path->v, t), vec3_dot(path->v, b), path->ndotv);
	wi_std.z = fmaxf(wi_std.z, G_EPSILON);
	wi_std = vec3_normalize(vec3(wi_std.x * alpha, wi_std.y * alpha, wi_std.z));

	t_vec3 wm_std = sample_vndf_hemisphere(wi_std, uv);
	t_vec3 wm = vec3_normalize(vec3(wm_std.x * alpha, wm_std.y * alpha, wm_std.z));
...
static inline t_vec3 sample_vndf_hemisphere(t_vec3 wi, t_vec2 uv) {
	...
	float cos_theta = (1.0f - uv.v) * (1.0f + wi.z) - wi.z;
	float sin_theta = sqrtf(fmaxf(0.0f, (1.0f - cos_theta * cos_theta)));
	t_vec3 c = spherical_to_cartesian(uv.u, sin_theta, cos_theta);
	t_vec3 h = vec3_add(c, wi);
	return h;
}
