/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   bsdf.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: myli-pen <myli-pen@student.hive.fi>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/25 22:49:46 by myli-pen          #+#    #+#             */
/*   Updated: 2026/03/25 22:49:47 by myli-pen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "materials.h"
#include "rendering.h"

float power_heuristic(float a, float b) {
	float a2 = a * a;
	float b2 = b * b;
	return a2 / fmaxf(a2 + b2, G_EPSILON);
}

t_vec3 bsdf(t_path* path) {
	t_vec3 diffuse = disney_brdf(path);
	t_vec3 specular = cook_torrance_brdf(path);
	return vec3_add(diffuse, specular);
}

float bsdf_pdf(t_path* path) {
	float pdf_d = pdf_cos(path->ndotl);
	float pdf_r = pdf_ggx_vndf(path);
	if (path->mat->metallic >= 0.9f)
		return pdf_r;
	return (path->p_spec * pdf_r) + ((1.0f - path->p_spec) * pdf_d);
}

bool sample_bsdf(t_path* path) {
	if (path->sample_spec) {
		sample_ggx_vndf(path, path->alpha, path->uv);
		path->l = vec3_reflect(path->ray.dir, path->h);
	} else {
		path->l = sample_cos_hemisphere(path->n, path->uv);
	}

	set_shader_data(path);
	if (path->ndotl <= LEN_SQ_EPSILON)
		return false;

	path->pdf = bsdf_pdf(path);
	if (path->pdf < G_EPSILON)
		return false;

	t_vec3 weight = vec3_scale(bsdf(path), path->ndotl / path->pdf);
	weight = vec3_clamp_mag(weight, CLAMP_INDIRECT);
	path->throughput = vec3_mul(path->throughput, weight);
	return true;
}
