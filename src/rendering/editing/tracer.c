...
bool	trace_ray_editing(const t_context *ctx, t_path *path, t_pixel *pixel)
{
	...
	if ((int)hit_object(ctx->editor.selected_obj, &path->ray, &path->hit) |
		...
		(int)hit_bvh_editing(ctx->scene.geo.bvh_root_idx, &path->ray, &path->hit, ctx->scene.geo.bvh_nodes) |
	...
	bg_color = vec3_scale(background_color(&ctx->scene, &path->ray, ctx->renderer.cam.skydome_uv_offset), ctx->scene.env.amb_light.intensity);
