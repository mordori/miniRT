...
static float	g_schlick_ggx(float dot, float alpha);
...
void	specular_probability(t_path *path, t_pixel *pixel, t_vec3 fresnel)
{
	...
	{
		path->p_spec = clampf(path->p_spec, 0.01f, 0.99f);
...
bool	bounce_specular(t_path *path, t_vec3 f0, float ndotv)
{
	...
	path->uv.v = clampf(path->uv.v, 0.01f, 0.99f);
	alpha = fmaxf(path->mat->roughness * path->mat->roughness, 0.00001f);
	h = sample_ggx(path->hit.normal, alpha, path->uv);
	...
	fresnel = vec3_schlick(f0, vdoth);
	g = g_schlick_ggx(ndotv, alpha) * g_schlick_ggx(ndotl, alpha);
	weight = (ndotl * vdoth) / (g * ndoth + G_EPSILON);
