...
static t_error parse_mat_values(char** tkns, t_material* mat) {
	...
	if (!parse_float(tkns[3], &mat->metallic) || !parse_float(tkns[4], &mat->roughness) || !parse_float(tkns[5], &mat->ior) ||
		!parse_float(tkns[6], &mat->transmission) || !parse_float(tkns[7], &em_str) || !parse_color(tkns[8], &em_col) ||
		!parse_uint(tkns[9], &mat->flags)) {
		return (E_INVALID_NUM);
	}
	if (!validate_range(mat->metallic, 0.0f, 1.0f) || !validate_range(mat->roughness, 0.0f, 1.0f) ||
		!validate_range(mat->ior, 1.0f, 3.0f) || !validate_range(mat->transmission, 0.0f, 1.0f) ||
